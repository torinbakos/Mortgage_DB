...
SELECT PropertyID, Address, Valuation, PropertyType,
...
SELECT m.LoanID, m.LoanAmount, SUM(p.AmountPaid) AS TotalPaid, (m.LoanAmount - SUM(p.AmountPaid)) AS CurrentBalance
FROM MortgageLoans m
...
SELECT b.BorrowerID, CONCAT(b.FirstName, ' ', b.LastName) AS FullName, COUNT(m.LoanID) AS LoanCount, AVG(m.LoanAmount) AS AverageLoanAmount
FROM Borrowers b
JOIN MortgageLoans m ON b.BorrowerID = m.BorrowerID
GROUP BY b.BorrowerID;
...
SELECT YEAR(m.StartDate) AS YearIssued, COUNT(m.LoanID) AS TotalLoansIssued, SUM(m.LoanAmount) AS TotalAmountIssued
FROM MortgageLoans m
GROUP BY YEAR(m.StartDate)
ORDER BY YearIssued;
...
SELECT m.LoanID, m.StartDate, m.LoanAmount,
...
SELECT m.BorrowerID, b.FirstName, b.LastName, m.LoanAmount, p.Valuation,
